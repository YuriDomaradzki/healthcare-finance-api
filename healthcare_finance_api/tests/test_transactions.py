...
class TestTransactionsResources(unittest.TestCase):
    ...
    def test_get_transactions_by_pharmacy_name_with_valid_name(self):
        name = 'Droga Mais'
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.get(f'{self.base_url}/transactions/pharmacy/{name}', headers=headers)
    ...
    def test_get_transactions_by_pharmacy_name_and_transaction_date_with_valid_name_and_date(self):
        name = 'DROGAO SUPER'
        transaction_date = '2020-02-05'
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.get(f'{self.base_url}/transactions/pharmacy/{name}/date/{transaction_date}', headers=headers)

        self.assertEqual(response.status_code, 200)

    def test_get_transactions_by_pharmacy_name_and_transaction_date_with_invalid_name_and_valid_date(self):
        name = 'Droga Mais_1'
        transaction_date = '2020-02-05'
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.get(f'{self.base_url}/transactions/pharmacy/{name}/date/{transaction_date}', headers=headers)
    ...
    def test_get_transactions_by_pharmacy_name_and_transaction_date_with_valid_name_and_invalid_date(self):
        name = 'Droga Mais'
    ...
    def test_get_transactions_by_patient_first_and_last_name_with_valid_first_and_last_name(self):
        first_name, last_name = 'Joana Silva'.split(" ")
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.get(f'{self.base_url}/transactions/patient/{first_name}/{last_name}', headers=headers)

        self.assertEqual(response.status_code, 200)

    def test_get_transactions_by_patient_first_and_last_name_with_invalid_first_name_and_valid_last_name(self):
    ...
    def test_get_transactions_by_patient_first_and_last_name_with_valid_first_name_and_invalid_last_name(self):
    ...
    def test_get_transactions_by_period_with_valid_first_and_last_date(self):
        ...
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.get(f'{self.base_url}/transactions/byPeriod/{start_date}/{end_date}', headers=headers)
    ...
    def test_get_transactions_by_values_with_valid_min_and_max_values(self):
        ...
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.get(f'{self.base_url}/transactions/byValues/{min_value}/{max_value}', headers=headers)
