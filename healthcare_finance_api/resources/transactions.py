    ...
    def get(self):
        try:
            transactions = [transaction.as_dict() for transaction in 
                            TransactionsModel.query.order_by(TransactionsModel.TIMESTAMP).all()
                            ]
            if not transactions:
...
class TranscationsByPharmacyName(MethodView):
    ...
    def get(self, name: str):
        ...
        try:
            transaction = [transaction.as_dict() for transaction in 
                           TransactionsModel.query.join(TransactionsModel.pharmacy)
...
class TranscationsByDate(MethodView):
    ...
    def get(self, name: str, date: str):
        ...
        try:
            ...
            if not transaction:
                raise LookupError(f"No transactions were found for the "\
                                  "pharmacy {name} in {date}")
            return {'Transaction': transaction}
...
class TransactionsByPatientFirstAndLastName(MethodView):
    ...
    def get(self, first_name: str, last_name: str):
        ...
        try:
            transaction = [transaction.as_dict() for transaction in 
                           TransactionsModel.query.join(TransactionsModel.patient)
            ...
            if not transaction:
                raise LookupError("No transactions were found for the patient"\
...
class TransactionsByPeriod(MethodView):
    ...
    def get(self, start_date: str, end_date: str):
        ...
        try:
            transaction = [transaction.as_dict() for transaction in 
                           TransactionsModel.query
                            .filter(and_(start_date <=
                                    func.date(TransactionsModel.TIMESTAMP),
                                    func.date(TransactionsModel.TIMESTAMP) <= end_date))
                            .order_by(TransactionsModel.TIMESTAMP)
                            .all()]
            if not transaction:
                raise LookupError("No transactions were found for in the "\
...
class TransactionsByValues(MethodView):
    ...
    def get(self, min_value: str, max_value: str):
        ...
        try:
            transaction = [transaction.as_dict() for transaction in 
                           TransactionsModel.query
                            .filter(TransactionsModel.AMOUNT.between(min_value, max_value))
                            .order_by(TransactionsModel.TIMESTAMP, TransactionsModel.AMOUNT)
                            .all()
                            ]
            if not transaction:
                raise LookupError(f"No transactions were found for the value range {min_value} to {max_value}")
            return {'Transaction': transaction}
