...
class PharmacyByName(MethodView):
    ...
    def get(self, name):
        ...
        try:
            pharmacy = [pharmacy.as_dict() for pharmacy in PharmacyModel.query.filter_by(NAME=name.upper()).all()]
            if not pharmacy:
...
class PharmacyByCity(MethodView):
    ...
    def get(self, city):
        ...
        try:
            pharmacy = [pharmacy.as_dict() for pharmacy in PharmacyModel.query.filter_by(CITY=city.upper()).all()]
            if not pharmacy:
...
class PharmacyByCityAndName(MethodView):
    ...
    def get(self, city, name):
        ...
        try:
            pharmacy = [pharmacy.as_dict() for pharmacy in PharmacyModel.query.filter_by(CITY=city.upper(), NAME=name.upper()).all()]
            if not pharmacy:
